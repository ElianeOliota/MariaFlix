...
class Crud:
    ...
    def select_custom(self, query,primaryKey_value=None):
        #self._cursor.execute ( select_query )
        select_query = sql.SQL ( query ).format ( )
        self._execute ( select_query, (primaryKey_value,) )
    ...
    def find(self, primaryKey_value=None):
        select_query = sql.SQL ( "SELECT row_to_json(a) FROM {} a WHERE {} = {}" ).format (
            sql.Identifier ( self.table ),
            sql.Identifier ( self.primarykey ),
            sql.Placeholder ()
        )
        self._execute ( select_query, (primaryKey_value,) )
        try:
            selected = self._cursor.fetchone ()
        except psycopg2.ProgrammingError as error:
            selected = '# ERROR: ' + str ( error )
        else:
            print ( '-# ' + str ( selected ) + '\n' )
    ...
    def select_all_json(self, primaryKey_value=None):
        if primaryKey_value == None:
            select_query = sql.SQL ( "SELECT row_to_json(a) FROM {} a" ).format ( sql.Identifier ( self.table ) )
            self._execute ( select_query )
        else:
            select_query = sql.SQL ( "SELECT * FROM {} WHERE {} = {}" ).format (
                sql.Identifier ( self.table ),
